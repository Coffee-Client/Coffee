...
public class Killaura extends Module {
    ...
    void doConfuse(Entity e) { // This also contains a range check
        ...
        if (!confuseAllowClip.getValue() && Objects.requireNonNull(CoffeeMain.client.world).getBlockState(new BlockPos(updatePos)).getMaterial().blocksMovement()) {
            return;
        }
    ...
    public void tick() {
        ...
        if (mode.getValue() == SelectMode.Single) {
            ...
            if (prio.getValue() == PriorityMode.Distance) {
                ...
                })).sorted(Comparator.comparingDouble(value -> value.getPos().distanceTo(Objects.requireNonNull(CoffeeMain.client.player).getPos()))).toList().get(0);
            }
            if (tar == null) {
                return;
            }
