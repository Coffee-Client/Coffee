...
public class MoreChatHistory extends Module {
    final DoubleSetting size = this.config.create(new DoubleSetting.Builder(300).name("Size").description("How big the new chat history should be allowed to getExtern (vanilla is 100)").min(10).max(1000).precision(0).get());
