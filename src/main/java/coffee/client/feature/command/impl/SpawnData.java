...
public class SpawnData extends Command {
    ...
    public ExamplesEntry getExampleArguments() {
        return new ExamplesEntry(
            "position 0 69 420  (sets the spawn position to 0 69 420)",
        ...
        switch (parser.consumeString().toLowerCase()) {
            case "position" -> {
                ...
                NbtGroup ng = new NbtGroup(new NbtObject(
                    "EntityTag",
                    new NbtList("Pos", new NbtProperty(parser.consumeDouble()), new NbtProperty(parser.consumeDouble()), new NbtProperty(parser.consumeDouble()))
                ));
                NbtCompound tag = ng.toCompound();
                stack.getOrCreateNbt().copyFrom(tag);
                CoffeeMain.client.player.networkHandler.sendPacket(new CreativeInventoryActionC2SPacket(
            ...
            case "velocity" -> {
                ...
                NbtGroup ng = new NbtGroup(new NbtObject(
                    "EntityTag",
                    new NbtList("Motion", new NbtProperty(parser.consumeDouble()), new NbtProperty(parser.consumeDouble()), new NbtProperty(parser.consumeDouble()))
                ));
                NbtCompound tag = ng.toCompound();
                stack.getOrCreateNbt().copyFrom(tag);
                CoffeeMain.client.player.networkHandler.sendPacket(new CreativeInventoryActionC2SPacket(
