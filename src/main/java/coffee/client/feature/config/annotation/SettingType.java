...
public enum SettingType {
    STRING(String.class,
            (setting, inputField, defaultValue) -> new StringSetting.Builder((String) inputField.get(defaultValue))), DOUBLE(
            double.class,
            (setting, inputField, defaultValue) -> {
                Utils.throwIfAnyEquals("Min, max and precision need to be defined",
                        -1,
                        setting.min(),
                        setting.max(),
                        setting.precision());
        ...
        return new DoubleSetting.Builder(inputField.getDouble(defaultValue)).min(setting.min())
            ...
            (setting, inputField, defaultValue) -> new BooleanSetting.Builder(inputField.getBoolean(defaultValue))), @SuppressWarnings("rawtypes") ENUM(
            Enum.class,
            (setting, inputField, defaultValue) -> new EnumSetting.Builder((Enum<?>) inputField.get(defaultValue))), COLOR(
            Color.class,
            (setting, inputField, declaringClass) -> new ColorSetting.Builder((Color) inputField.get(declaringClass)));
    @Getter
