...
public class TextFieldElement extends Element implements HasSpecialCursor {
    ...
    public void render(MatrixStack stack, double mouseX, double mouseY) {
        ...
        Renderer.R2D.renderRoundedQuad(stack, new Color(40, 40, 40), getPositionX(), getPositionY(), getPositionX() + width,
                getPositionY() + height, 5, 20);
        if (isInvalid()) {
            Renderer.R2D.renderRoundedOutline(stack, Color.RED, getPositionX() + .5, getPositionY() + .5, getPositionX() + width - .5,
                    getPositionY() + height - .5, 5, 5, 5, 5, .5, 20);
        }

        ClipStack.globalInstance.addWindow(stack,
                new Rectangle(getPositionX() + pad, getPositionY(), getPositionX() + width - pad, getPositionY() + height));
        // Text content
        if (!text.isEmpty()) {
            FontRenderers.getRenderer()
                    .drawString(stack, text, (float) (getPositionX() + pad - overflowWidth), (float) (centerY), 0xFFFFFF, false);
        } else {
            FontRenderers.getRenderer()
                    .drawString(stack, suggestion, (float) (getPositionX() + pad - overflowWidth), (float) (centerY), 0xAAAAAA, false);
        }
        ...
        if (focused && (cursor != selectionStart || cursor != selectionEnd)) {
            ...
            Renderer.R2D.renderQuad(stack, new Color(50, 50, 255, 100), selStart, centerY, selEnd,
        ...
        if (focused && renderCursor) {
            Renderer.R2D.renderQuad(stack, Color.WHITE, getPositionX() + pad + getTextWidth(cursor) - overflowWidth, centerY,
