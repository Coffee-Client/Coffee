...
public class NotificationRenderer {
    ...
    public static void onFastTick() {
        ...
        for (Notification notification : new ArrayList<>(topBarNotifications)) {
            ...
            if (notification.shouldDoAnimation) {
                notification.animationProgress = Transitions.transition(notification.animationProgress, notification.animationGoal, 10,
                        0.0001);
            }
        }
    }
    ...
    public static void renderTop() {
        ...
        for (Notification notification : nf) {
            ...
            Renderer.R2D.renderQuad(ms, topBg, notification.renderPosX - width, notification.renderPosY,
            ...
            if (notification.duration == -1) {
                ...
                Renderer.R2D.renderQuad(ms, ThemeManager.getMainTheme().getActive(), notification.renderPosX - width,
                        notification.renderPosY + height - 1, notification.renderPosX + width + 1, notification.renderPosY + height);
                Renderer.R2D.renderQuad(ms, ThemeManager.getMainTheme().getAccent(), notification.renderPosX - width,
                        notification.renderPosY + height - 1, notification.renderPosX - width + ((width + 1) * 2 * timeRemainingInv),
    ...
    public static void renderSide() {
        ...
        long c = System.currentTimeMillis();
        notifications.removeIf(notification -> notification.creationDate + notification.duration < c && Transitions.easeOutExpo(
        ...
        for (Notification notification : new ArrayList<>(notifications)) {
            ...
            double notificationHeight = Math.max(iconDimensions,
                    contentHeight) + texPadding * 2d; // always have padding at the outside no matter what
            ...
            Renderer.R2D.renderRoundedQuad(ms, new Color(20, 20, 20, (int) Math.min(255, 255 * interpolatedAnimProgress)), notificationX,
                    notificationY, notificationX + notificationWidth, notificationY + notificationHeight, 5, 20);
            ...
            Renderer.R2D.renderTexture(ms, notificationX + texPadding, notificationY + notificationHeight / 2d - iconDimensions / 2d,
                    iconDimensions, iconDimensions, 0, 0, iconDimensions, iconDimensions, iconDimensions, iconDimensions);
