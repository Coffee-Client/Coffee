...
public class NameTags extends Module {
    ...
    void drawInternal(Vec3d screenPos, String text, AbstractClientPlayerEntity entity) {
        ...
        infoDrawer.drawString(stack1, gmString, actual.x + width / 2d - infoDrawer.getStringWidth(gmString) - 2, actual.y + 2 + nameDrawer.getFontHeight(),
                0xAAAAAA);
        if (ping != -1) {
            infoDrawer.drawString(stack1, pingStr, actual.x - width / 2d + 2, actual.y + 2 + nameDrawer.getFontHeight(), 0xAAAAAA);
        }
        Renderer.R2D.renderRoundedQuad(stack1, new Color(60, 60, 60, 255), actual.x - width / 2d + 2, actual.y + labelHeight - 2 - healthHeight,
                actual.x + width / 2d - 2, actual.y + labelHeight - 2, healthHeight / 2d, 10);
