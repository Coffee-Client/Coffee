...
public class TargetHud extends Module {
    ...
    final BooleanSetting renderPing = this.config.create(
            new BooleanSetting.Builder(true).name("Render ping").description("Shows the ping of the enemy").get());
    final BooleanSetting renderHP = this.config.create(
            new BooleanSetting.Builder(true).name("Render health").description("Shows the HP of the enemy").get());
    double wX = 0;
    double renderWX1 = 0;
    Entity e = null;
    Entity re = null;
    ...
    public void tick() {
        ...
        List<Entity> entitiesQueue = StreamSupport.stream(Objects.requireNonNull(CoffeeMain.client.world).getEntities().spliterator(),
                        false)
    ...
    public void draw(MatrixStack stack) {
        ...
        if (re != null) {
            ...
            double pillHeight = 2;
            Renderer.R2D.renderRoundedQuad(stack, new Color(0, 0, 0, 200), textLeftAlign, modalHeight - 5 - pillHeight, modalWidth - 5,
                    modalHeight - 5, pillHeight / 2d, 10);
            Renderer.R2D.renderRoundedQuad(stack, MID_END, textLeftAlign, modalHeight - 5 - pillHeight, renderToX, modalHeight - 5,
                    pillHeight / 2d, 10);
