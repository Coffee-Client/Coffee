...
public class SurvivalNuker extends Module {
    ...
    final DoubleSetting range = this.config.create(
            new DoubleSetting.Builder(4).name("Range").description("How far to break blocks").min(0).max(4).precision(1).get());
    ...
    final DoubleSetting delay = this.config.create(
            new DoubleSetting.Builder(0).name("Delay").description("How much to wait between ticks").min(0).max(20).precision(0).get());
    final BooleanSetting ignoreXray = this.config.create(
            new BooleanSetting.Builder(true).name("Ignore XRAY").description("Ignores XRAY blocks").get());
    final EnumSetting<Mode> mode = this.config.create(
            new EnumSetting.Builder<>(Mode.Everything).name("Mode").description("What to break").get());
    final BooleanSetting autoTool = this.config.create(
            new BooleanSetting.Builder(true).name("Auto tool").description("Automatically picks the best tool for the block").get());
    final EnumSetting<SortMode> mv = this.config.create(
            new EnumSetting.Builder<>(SortMode.OutIn).name("Sorting").description("In which order to break the blocks").get());
    final BooleanSetting ignoreUnbreakable = this.config.create(
            new BooleanSetting.Builder(true).name("Ignore unbreakable").description("Ignores unbreakable blocks").get());
    int delayPassed = 0;
    ...
    public void tick() {
        ...
        for (BlockPos np : toHit) {
            ...
            if (!bs.isAir() && bs.getBlock() != Blocks.WATER && bs.getBlock() != Blocks.LAVA && !isUnbreakable(
                    bs.getBlock()) && b && client.world.getWorldBorder().contains(np) && isBlockApplicable(bs.getBlock())) {
                renders.add(np);
    ...
    public void onWorldRender(MatrixStack matrices) {
        for (BlockPos render : renders) {
            ...
            Renderer.R3D.renderFadingBlock(Renderer.Util.modify(Utils.getCurrentRGB(), -1, -1, -1, 255),
                    Renderer.Util.modify(Utils.getCurrentRGB(), -1, -1, -1, 100).darker(), vp, new Vec3d(1, 1, 1), 1000);
        }
    }
